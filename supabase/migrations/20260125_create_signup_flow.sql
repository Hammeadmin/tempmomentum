-- Migration: Create complete_organization_signup RPC function
-- Purpose: Enable "Genesis Block" capability - first Admin (CEO) can create first Organization during signup
-- This function is called immediately after supabase.auth.signUp() to complete the registration flow
...
BEGIN
  ...
  -- Step 4: Create the user profile with admin role
...
GRANT EXECUTE ON FUNCTION public.complete_organization_signup(text, text, text) TO authenticated;

-- Revoke from anon to ensure only logged-in users can call this
REVOKE EXECUTE ON FUNCTION public.complete_organization_signup(text, text, text) FROM anon;
...
Parameters:
  ...
  - org_number: The organisation registration number
