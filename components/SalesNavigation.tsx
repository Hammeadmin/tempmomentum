// src/components/SalesNavigation.tsx
...
function SalesNavigation({ collapsed, onToggle }: SalesNavigationProps) {
  const location = useLocation();
  const { signOut, user } = useAuth();

  const navigation = [
    { name: 'Dashboard', href: '/sales-dashboard', icon: Home },
    { name: 'Säljtunnel', href: '/Säljtunnel', icon: TrainFrontTunnel },
    { name: 'Förfrågningar', href: '/leads', icon: Users },
    ...
    { name: 'Orderhantering', href: '/Orderhantering', icon: Package },
            ...
            <div className="flex items-center space-x-3 p-3 bg-blue-50 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">
                  {user?.email?.split('@')[0] || 'Säljare'}
                </p>
                <p className="text-xs text-blue-600">Säljare</p>
              </div>
            </div>
          </div>
        )}
